...
builder.Services.AddAuthentication(CookieAuthenticationDefaults.AuthenticationScheme)
    ...
    {
        options.Cookie.Name = "OrigamiHelperLoginCookie";
...
builder.Services.AddIdentityCore<IdentityUser>(config =>
    ...
    .AddEntityFrameworkStores<OrigamiHelperDbContext>();

// allows passing datetimes without time zone data 
AppContext.SetSwitch("Npgsql.EnableLegacyTimestampBehavior", true);

// allows our api endpoints to access the database through Entity Framework Core
builder.Services.AddNpgsql<OrigamiHelperDbContext>(builder.Configuration["OrigamiHelperDbConnectionString"]);
...
app.UseStaticFiles(); // enables wwwroot to serve files
